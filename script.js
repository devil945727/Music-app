...
async function getSongs(folder) {
    ...
    // Fetch songs in the folder using relative paths
    let response = await fetch(`./${folder}`);
    let textResponse = await response.text();

    // Parse response to find song links
...
const playMusic = (track, pause = false) => {
    // Use relative paths for playing songs
    currentsong.src = `./${currfolder}/` + track;

    if (!pause) {
        currentsong.play();
        document.querySelector("#play").src = "./img/svg/pause.svg";
    }
...
async function displayAlbum() {
    // Fetch album data from the songs folder
    ...
    for (let index = 0; index < array.length; index++) {
        ...
        if (e.href.includes("/songs/")) {
            ...
            let albumInfo = await metadata.json();
    ...
    // Add event listeners to load songs when album is clicked
...
async function main() {
    ...
    document.querySelector("#play").addEventListener("click", () => {
        if (currentsong.paused) {
            ...
            currentsong.pause();
            document.querySelector("#play").src = "./img/svg/play.svg";
        }
    });
